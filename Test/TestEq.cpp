...
//Create discretization
	...
	shared_ptr<const ADiscretization<TEST_OP_TYPE, 1>> d = \
		make_shared<const Collocation<TEST_OP_TYPE, 1, double>>(bases);

//============================================================

	op = new TestOperator(d);
	TestOperator &op_ = dynamic_cast<TestOperator &>(*op);

	sol = make_shared<Function<TEST_OP_TYPE, 1>>(d);
	AFunction<double> *rhs_ = \
		new Function<TEST_OP_TYPE, 1>(d);
...
//double gen problem
	...
	rhs_->coef[0] = 0.754686681982361; rhs_->coef[1] = 0.276025076998578; rhs_->coef[2] = 0.679702676853675; rhs_->coef[3] = 0.655098003973841; rhs_->coef[4] = 0.162611735194631; rhs_->coef[5] = 0.118997681558377; rhs_->coef[6] = 0.498364051982143;
	rhs = std::move(rhs_);
/*
...
//complex gen problem
	...
	rhs_->coef[0] = Complex(0.01, 0.60); rhs_->coef[1] = Complex(0.33, 0.26); rhs_->coef[2] = Complex(0.16, 0.65); rhs_->coef[3] = Complex(0.79, 0.68); rhs_->coef[4] = Complex(0.31, 0.74); rhs_->coef[5] = Complex(0.52, 0.45); rhs_->coef[6] = Complex(0.16, 0.08);
	rhs = std::move(rhs_);
	*/
/*
